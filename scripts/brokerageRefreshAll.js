...
const refreshBrokerageForAllLevel7Users = async () => {
  try {
    ...
    // Filter for level 7 users
    const filteredLevel7Users = level7Users.filter(user => user.accountType?.level === 7);
    
    if (!filteredLevel7Users || filteredLevel7Users.length === 0) {
      console.log('No level 7 users found');
      return;
    }
    
    console.log(`Found ${filteredLevel7Users.length} level 7 users\n`);
    ...
    // Process each user
    for (const user of filteredLevel7Users) {
      console.log(`\n${'='.repeat(60)}`);
    ...
    console.log('SUMMARY');
    console.log(`${'='.repeat(60)}`);
    console.log(`Total Users Processed: ${filteredLevel7Users.length}`);
